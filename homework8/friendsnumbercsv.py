...
def main():
    ...
    try:
        with open("./friendData.csv","r", encoding='utf-8', newline='') as f:
            while True:
                line = csv.reader(f)
                if not line:
                    break
    ...
    while True:
        ...
        if user == 1:
            ...
            with open("./friendData.csv", "w", encoding='utf-8', newline="") as f:
                for key in address_book:
                    w = csv.writer(f)
                    w.writerow(key + ',')
                    w.writerow(address_book[key][0] + ',')
                    w.writerow(address_book[key][1] + ',')
                    w.writerow(address_book[key][2] + ',')
                    f.close()
