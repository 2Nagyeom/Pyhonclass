...
def main():
    ...
    try:
        with open("./friendData.json","r", encoding='UTF-8') as f:
            while True:
                line = json.load(f)
                if not line:
                    break
                print(json.dump(line, indent="\t"))
    ...
    while True:
        ...
        if user == 1:
            ...
            for key in sorted(address_book):
                info = address_book.get(key)
                print(json.dumps(key, "의 전화번호 : ", info[0],key, "의 주소 : ", info[1],key, "의 이메일 : ", info[2], indent="\t"))

        else:
            with open("./friendData.json", "w", encoding='UTF-8-sig') as f:
